 ...
 */
require KROTH_ADMIN_PATH .'/cs-framework.php';
require KROTH_FRAMEWORK .'/theme-options/theme-options.php';
require KROTH_FRAMEWORK .'/theme-options/theme-customizer.php';
require KROTH_FRAMEWORK .'/theme-options/theme-metabox.php';
 ...
 */
function kroth_vt_cs_framework_override() {
  return 'inc/theme-options/theme-extend';
}
add_filter( 'cs_framework_override', 'kroth_vt_cs_framework_override' );

/**
 * Add Extra Icons Font in Icon Picker
 */
function kroth_vt_add_icons_json( $json = array() ) {
  $json[] = KROTH_CS_FRAMEWORK .'/fields/icon/simple-line-icons.json';
  return $json;
}
add_filter( 'cs_add_icons_json', 'kroth_vt_add_icons_json' );

/**
 * Custom New Font Family
 */
if( ! function_exists( 'kroth_vt_custom_font_upload' ) ) {
  function kroth_vt_custom_font_upload( $db_value ) {
  ...
  add_action( 'cs_typography_family', 'kroth_vt_custom_font_upload' );
}

/**
 * Check Custom Font
 */
if ( ! function_exists( 'kroth_vt_custom_upload_font' ) ) {
  function kroth_vt_custom_upload_font( $font ) {
 ...
 */
if ( ! function_exists( 'kroth_vt_registered_sidebars' ) ) {
  function kroth_vt_registered_sidebars() {
 ...
 */
if ( ! function_exists( 'kroth_vt_google_fonts' ) ) {
  function kroth_vt_google_fonts() {
    ...
    if ( ! empty( $embed_fonts ) ) {
      ...
      wp_enqueue_style( 'kroth-google-fonts', esc_url( add_query_arg( 'family', urlencode( implode( '|', $query_args ) ) . $subsets, '//fonts.googleapis.com/css' ) ), array(), null );
    }
...
/* Typography */
if ( ! function_exists( 'kroth_vt_get_typography' ) ) {
  function kroth_vt_get_typography() {
    ...
    if ( ! empty( $typography ) ) {
      foreach ( $typography as $font ) {
        if ( ! empty( $font['selector'] ) ) {
          ...
          // $weight  = ( $font['font']['variant'] != 'regular' ) ? kroth_vt_esc_string( $font['font']['variant'] ) : 400;
          $style   = kroth_vt_esc_number( $font['font']['variant'] );
          ...
          $output .= ( ! empty( $font['size'] ) ) ? 'font-size: '. kroth_check_px( $font['size'] ) .';' : '';
